...
Variable pool2d(
  ...
  std::vector<long int> paddingDims = {static_cast<long int>(py),
                                       static_cast<long int>(px)};
  ...
  fwdPoolingArgs.insert({DNNL_ARG_SRC, inputMemory});
  fwdPoolingArgs.insert({DNNL_ARG_DST, outputMemory});
  if (input.isCalcGrad()) {
    workspaceMemory =
        std::make_shared<memory>(primDesc.workspace_desc(), dnnlEngine);
    pooling = std::make_shared<pooling_forward>(primDesc);
    // inputMemory, outputMemory, *workspaceMemory);
    fwdPoolingArgs.insert({DNNL_ARG_WORKSPACE, *workspaceMemory});
  } else {
    pooling = std::make_shared<pooling_forward>(primDesc);
